...
export default function Support() {
  ...
  const fetchTermsData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setTermsData({
          title: response.data.data?.title || 'Support',
          content: textContent
        })
      } else {
        setTermsData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Support details for ${role} updated successfully`)
        const content = response.data.data?.content || ''
        const textContent = legalHtmlToPlainText(content)
        setTermsData({
          title: response.data.data?.title || 'Support',
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">Manage Support details for different user roles</p>
          </div>
        ...
        {loading ? (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-20 flex flex-col items-center justify-center">
            ...
            <p className="mt-4 text-slate-600 font-medium">Loading {role} support...</p>
          </div>
        ) : (
          <>
            {/* Text Area */}
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <div className="text-sm text-slate-500 italic">
                  Editing support for <span className="font-bold text-blue-600 capitalize">{role}</span>
                </div>
