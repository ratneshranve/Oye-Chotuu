...
import { 
    Search, Filter, PlayCircle, PauseCircle, 
    User, Clock, MoreVertical, Loader2, ArrowUpDown,
    AlertCircle, Info, Eye, Milk, Calendar, RefreshCw
} from 'lucide-react';
...
const PausedPlans = () => {
    ...
    const fetchPlans = async () => {
        try {
            ...
            toast.error('Failed to load paused plans');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-slate-500">Managing temporarily suspended delivery plans.</p>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="flex-1 max-w-md relative">
                            ...
                            <Input 
                                placeholder="Search paused users..." 
                                className="pl-10 border-slate-200 rounded-xl h-11 focus:ring-amber-500 focus:border-amber-500"
                        ...
                        <Button 
                            onClick={fetchPlans} 
                            className="rounded-xl h-11 bg-amber-600 text-white hover:bg-amber-700 transition-all px-8 shadow-sm font-bold"
                        >
                            Search
                        </Button>
                    </div>
                        ...
                        <Table>
                            <TableHeader className="bg-slate-50/50">
                                <TableRow className="border-slate-100">
                                    ...
                                    <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider px-6 py-4">Pause Date</TableHead>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-32 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <p className="text-xs text-slate-400 font-medium">Fetching paused plans...</p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ) : plans.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={6} className="h-32 text-center">
                                            <div className="flex flex-col items-center gap-2 text-slate-500">
                                                ...
                                                <p className="text-sm font-medium">No paused subscriptions found</p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    plans.map((plan) => (
                                        <TableRow key={plan._id} className="border-slate-50 hover:bg-slate-50/50 transition-colors">
                                            <TableCell className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center text-amber-600 font-bold border border-amber-100 shadow-sm">
                                            ...
                                            <TableCell className="px-6 py-4 text-center">
                                                <div className="inline-flex flex-col items-center">
                                                    <span className="text-sm font-black text-amber-600">{plan.remainingDays}</span>
