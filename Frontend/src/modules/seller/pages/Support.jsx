...
export default function Support() {
  ...
  const fetchTermsData = async () => {
    try {
      setLoading(true)
      const response = await api.get(`${API_ENDPOINTS.ADMIN.SUPPORT_PUBLIC}?role=vendor`)
        ...
        <div className="max-w-4xl mx-auto px-4 h-16 md:h-20 flex items-center gap-4">
          ...
          <div className="flex-1">
             <h1 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white tracking-tight leading-none">
               {termsData.title || "Vendor Support"}
             </h1>
        ...
        >
          {termsData.content ? (
            ...
            <div className="text-center py-20">
               ...
               <p className="text-gray-400 font-medium">No vendor support available at the moment.</p>
            </div>
          )}
        </motion.div>
